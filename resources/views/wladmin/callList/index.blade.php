...
<div class="content-wrapper pb-0">
    ...
    if($CompanyMaster->iAllowedCallCount == 0){
        ...
        $options = ['NO', 'ID', 'LEVEL', 'STATUS', 'CALL COUNT', 'DATE', 'SYSTEM', 'COMPONENT', 'SUPPORT TYPE', 'CUSTOMER', 'COMPANY', 'PROJECT', 'ACTIONS'];
        $optionLabels = [
            'NO' => 'No',
            'ID' => 'ID',
            'LEVEL' => 'Level',
                        ...
                        <tr>
                            ...
                            <?php if (in_array('ID', $selectedOptions)) {
                                if (isset($ticket->strTicketUniqueID) && $ticket->strTicketUniqueID != '') {
                                    echo "<td><a href=\"" . route('callList.info', $ticket->iTicketId) . "\" title=\"Info\" class=\"table-action fs-6 text-secondary text-decoration-underline\">" . $ticket->strTicketUniqueID . "</a></td>";
                                } else {
                                    echo "<td><a href=\"" . route('callList.info', $ticket->iTicketId) . "\" title=\"Info\" class=\"table-action fs-6 text-secondary text-decoration-underline\">" . str_pad($ticket->iTicketId, 4, '0', STR_PAD_LEFT) ."</a></td>";
                                }
                            } ?>
...
<?php if (in_array('46', \Session::get('menuList'))) { ?>
    <script>
        ...
        $(function() {
            $('.mas-download').click(function(e) {
                ...
                $("#fresh-table").table2excel({
                    name: "Call List",
                    filename: "Call_List_" + new Date().toISOString().replace(/[\-\:\.]/g, ""),
                    fileext: ".xls"
                });
        ...
        function clearData() {
            window.location.href = "{{ route('callList.index') }}";
        }
    </script>
<?php } else { ?>
    <script>
