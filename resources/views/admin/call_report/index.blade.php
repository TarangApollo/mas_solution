...
@section('content')
    ...
    <div class="main-panel">
        <div class="content-wrapper pb-0">
            ...
            $options = ['ID', 'NO', 'OUTCOME', 'STATUS','CALL COUNT', 'DATE', 'AUDIO', 'SERVICE','PROJECT', 'SYSTEM', 'PROJECTTYPE', 'OPEN LEVEL', 'CLOSE LEVEL', 'DURATION', 'ATTENDANT', 'ACTIONS'];
            $optionLabels = [
                'ID' => 'No',
                'NO' => 'Id',
                'OUTCOME' => 'Outcome',
                                    ...
                                    <tr>
                                        ...
                                        <?php if (in_array('ID', $selectedOptions)) {
                                            if (isset($ticket->strTicketUniqueID) && $ticket->strTicketUniqueID != '') {
                                                echo "<td><a href=\"" . route('call_report.info', $ticket->iTicketId) . "\" title=\"Info\" class=\"table-action fs-6 text-secondary text-decoration-underline\">" . $ticket->strTicketUniqueID . "</a></td>";
                                            } else {
                                                echo "<td><a href=\"" . route('call_report.info', $ticket->iTicketId) . "\" title=\"Info\" class=\"table-action fs-6 text-secondary text-decoration-underline\">" . str_pad($ticket->iTicketId, 4, '0', STR_PAD_LEFT) ."</a></td>";
                                            }
                                        } ?>
                                        ...
                                        <?php } ?>
                                        <?php if (in_array('CALL COUNT', $selectedOptions)) {
                                            echo "<td>";
                                            if(isset($ticket->iPhoneStatus) && $ticket->iPhoneStatus == 1){
                                                echo "Phone :{$ticket->iPhoneCount}";
                                            }
                                            if(isset($ticket->iWhatsAppStatus) && $ticket->iWhatsAppStatus == 1){
                                                echo "<br /> WhatsApp :{$ticket->iWhatsAppCount}";
                                            }
                                            echo "</td>";
                                        } ?>
                                        ...
                                        <td>
                                            ...
                                            <form action="{{ route('call_report.ticket_delete', $ticket->iTicketId) }}" method="POST" onsubmit="return confirm('Are you Sure You wanted to Delete?');" style="display: inline-block;">
