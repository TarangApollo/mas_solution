                            ...
                            <div class="accordion-box-content">
                                <div class="tab-content clearfix">
                                    ...
                                    <div class="tab-pane fade in active" id="support">
                                        <h3 class="tab-content-title">Allow Call Count</h3>
                                        <form class="was-validated pb-3" name="frmparameter" id="frmparameter" action="{{route('company.allowcallcountstore')}}" method="post">
                                            ...
                                            <div class="row">
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        <label>Allow Call Count</label>
                                                        <select class="js-example-basic-single" style="width: 100%;"
                                                            name="iAllowedCallCount" id="iAllowedCallCount">
                                                            <option label="Please Select" value="">--Select --</option>
                                                            <option value="1" @if($companyMasterModules->iAllowedCallCount == 1) {{ 'selected' }} @endif>Yes</option>
                                                            <option value="0" @if($companyMasterModules->iAllowedCallCount == 0) {{ 'selected' }} @endif>No</option>
                                                        </select>
...
<script>
    $('#submit').on("click", function() {
        ...
        $.ajax({
            type: 'POST',
            url: "{{route('company.allowcallcountstore')}}",
            ...
            success: function(response) {
                ...
                if (response > 0) {
                    $('#loading').css("display", "none");
                    $("#successalert").show();
                    $("#msgdata").html("<strong>Success !</strong> Call Counts Allowed Successfully.");
                    $('#company_id').val(response);
                    var company_id = response;
                    $('#save').val('0');

                    // var url = "{{route('company.allow-call-count',':company_id')}}";
