...
class CallReportController extends Controller
{
    public function index(Request $request)
    {
        if (Auth::User()->role_id == 1) {
            ...
            $systems = System::where(['isDelete' => 0, 'iStatus' => 1])->distinct()
                ->when($request->OEMCompany, fn ($query, $OEMCompany) => $query->where('iCompanyId', $OEMCompany))
                ->when($request->iSystemId, fn ($query, $iSystemId) => $query->where('iSystemId', $iSystemId))
                ->orderBy('strSystem', 'ASC')->get();
            ...
            $secondQuery = DB::table('users as u')
                ...
                ->where(function ($q) {
                    ...
                    ->where(function ($q2) {
                        $q2->whereIn('u.role_id', [2,3])
                           ->where('u.isCanSwitchProfile', 1);
                    });
                });
            
            // ===============================
            // UNION + DISTINCT (WRAPPED)
            // ===============================
            ...
            $supporttypes = SupportType::where(['isDelete' => 0, 'iStatus' => 1])
                ->when($request->OEMCompany, fn ($query, $OEMCompany) => $query->where('iCompanyId', $OEMCompany))
                ->when($request->iSupportType, fn ($query, $iSupportType) => $query->where('iSuppotTypeId', $iSupportType))
                ->orderBY('strSupportType', 'ASC')->get();
    ...
    function getCallAttendant(Request $request)
    {
        ...
        //     })
        //     ->get();
        $companyId = $request->OEMCompany; // = 10
    ...
    public function ticket_delete(Request $request,$id){
        try {
            ...
            $galleryFiles = DB::table('ticketdetail')
                ->where('iTicketId', $id)
                ->get();
            
            foreach ($galleryFiles as $file) {

                $imagePath  = public_path('ticket_images/' . $file->DocumentName);
                $videoPath  = public_path('ticket_video/' . $file->DocumentName);
                $docPath    = public_path('ticket_images/' . $file->DocumentName);
                ...
                // Delete document (pdf/doc/etc)
                if (file_exists($docPath)) {
                    unlink($docPath);
                }
            }
            
            DB::table('ticketdetail')
            ->where('iTicketId', $id)
            ->delete();
            
            DB::table('ticketlog')->where([
                "iticketId" => $id
            ])->delete();
            
            DB::table('ticketmaster')->where([
                "iTicketId" => $id
            ])->delete();
