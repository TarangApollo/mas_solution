...
class WlUserController extends Controller
{
    ...
    public function delete($Id)
    {
        if (Auth::User()->role_id == 2) {
            ...
            DB::table('callattendent')->where("iUserId", $Id)->update(["isDelete" => 1]);
    ...
    public function updateStatus(Request $request)
    {
        if (Auth::User()->role_id == 2) {
            if ($request->status == 1) {
                ...
                $callattendent = DB::table('callattendent')->where("iUserId", $request->UserId)->first();
                if($callattendent){
                    DB::table('callattendent')->where("iUserId", $request->UserId)->update(['iStatus' => 0]);
                }
            } else {
                ...
                $callattendent = DB::table('callattendent')->where("iUserId", $request->UserId)->first();
                if($callattendent){
                    DB::table('callattendent')->where("iUserId", $request->UserId)->update(['iStatus' => 1]);
                }
            }
            echo 1;
        } else {
