     ...
     public function wl_index(Request $request)
    {
        ...
        // try {
            ...
            if (Auth::User()->role_id == 2)
            {
                ...
                if (isset($request->fYear) && $request->fYear != "") {
                    $rmsDatas->whereBetween('rma.strRMARegistrationDate', [$datefrom, $dateto]);
                }
                if($request->status == "TotalOpenRma"){
                    $rmsDatas->where('strStatus', 'Open');
                }
                $rmsDatas->when($searchMonth, function ($query, $month) {
                    $query->whereMonth('rma.strRMARegistrationDate', $month);
                })
