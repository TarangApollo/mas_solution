...
class RMAController extends Controller
{
    ...
    public function rma_info(Request $request, $id)
    {
        ...
        return view('call_attendant.rma.info', compact('Rma_info_log', 'rmaList', 'Documents', 'rmadetailList', 'id', 'systemList'));
    }
    public function rma_edit(Request $request)
    {
        ...
        if ($response->Factory_Status == 'HFI') {
            $HFI_Status_html .= '<option value="HFI" selected>HFI</option>';
        } else {
            $HFI_Status_html .= '<option value="HFI">HFI</option>';
        }
        //--------------------------field 15 end ----------------------------
    ...
    public function wl_index(Request $request)
    {
        ...
        if (Auth::User()->role_id == 2) {
            ...
            $rmsDatas = DB::table('rma')
                ->select('rma.*', 'ticketmaster.CustomerName')
                ->leftJoin('ticketmaster', 'rma.iComplaintId', '=', 'ticketmaster.iTicketId')
            ...
            return view('wladmin.Wl_RMA.index', compact('searchMonth', 'searchYear', 'yearList', 'rmsData', 'search_Customer_Name_and_RMA_No', 'search_Customer_Status'));
        } else {
            return redirect()->route('home');
        }
    }
    public function rma_summary_info(Request $request, $id = null)
    {
        ...
        $Rma_info_log = DB::table('rma_infolog')
            ...
            ->where('tableName', 'rma')
            ->get();
        return view('wladmin.Wl_RMA.rma-summary-info', compact('Rma_info_log', 'rmaList', 'rmadetailList', 'Documents',));
    }
    public function rma_month_summary_index(Request $request)
    {
        if (Auth::User()->role_id == 2) {
            $yearList = DB::table('yearlog')->orderBy('iYearId', 'DESC')->get();
            $register_count = DB::table('rma')
                ...
                ->groupByRaw('MONTHNAME(strRMARegistrationDate)')
                ->get();
            return view('wladmin.Wl_RMA.rma_month_summary', compact('yearList', 'register_count'));
        } else {
            return redirect()->route('home');
        }
    }
    ...
    public function rma_month_call_info(Request $request, $id = null)
    {
        ...
        if (Auth::User()->role_id == 2) {
            ...
            return view('wladmin.Wl_RMA.rma_month_call_info', compact('Rma_info_log', 'rmaList', 'rmadetailList', 'Documents'));
        } else {
            return redirect()->route('home');
        }
    }
