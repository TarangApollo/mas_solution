     ...
     public function index(Request $request)
    {
        try {
            if (Auth::User()->role_id == 3) {
                ...
                return view('call_attendant.rma.add', compact('CompanyMaster', 'ticketList', 'systemList', 'distributorList', 'iRMANumber', 'rmaList'));
            } else {
            ...
            if (Auth::User()->role_id == 3) {
               ...
               // Generate dynamic RMA number
                ...
                if (isset($companyid[0]->strCompanyPrefix) && $companyid[0]->strCompanyPrefix != "") {
                    ...
                    $iRMANumber = $companyid[0]->strCompanyPrefix . "-RN" . str_pad($iOEM_RMA_Id, 5, '0', STR_PAD_LEFT);
                }
    ...
    public function wl_index(Request $request)
    {
        try {
            if (Auth::User()->role_id == 2)
            {
                ...
                return view('wladmin.Wl_RMA.index', compact('HFI', 'searchMonth', 'searchYear', 'yearList', 'rmsData', 'search_Customer_Name_and_RMA_No', 'search_Customer_Status'));
            } else {
                return redirect()->route('home');
            }
        } catch (\Exception $e) {
     ...
     public function rma_month_summary_index(Request $request)
    {
        try {
            if (Auth::User()->role_id == 2) {
                ...
                if (isset($request->fYear) && $request->fYear != "") {
                    $query->whereBetween('rma.created_at', [$datefrom, $dateto]);
                }
