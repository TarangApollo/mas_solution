    ...
    public function index(Request $request)
    {
        if (Auth::User()->role_id == 3) {
            ...
            $systemLists = System::where(['system.isDelete' => 0, 'system.iStatus' => 1])->orderBy('strSystem', 'ASC')
                ->when($search_company, fn($query, $search_company) => $query->where('iCompanyId', $search_company))
                ->get();
            $Faqs = Reference::select('reference.*', 'component.strComponent', 'subcomponent.strSubComponent', 'system.strSystem', 'companymaster.strOEMCompanyName')
                ->orderBy('iRefId', 'DESC')
                ...
                ->where(['reference.isDelete' => 0, 'reference.iStatus' => 1])->get();
