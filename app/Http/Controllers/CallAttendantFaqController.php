    ...
    { 
        if (Auth::User()->role_id == 3) {
            ...
            $systemLists = System::where(['system.isDelete' => 0, 'system.iStatus' => 1])->orderBy('strSystem', 'ASC')
                ->when($search_company, fn($query, $search_company) => $query->where('iCompanyId', $search_company))
                ->get();
            $Faq = Faq::select('faqmaster.*', 'companymaster.strOEMCompanyName', 'component.strComponent', 'subcomponent.strSubComponent', 'system.strSystem')->orderBy('iFAQId', 'DESC')
                ...
                ->where(['faqmaster.isDelete' => 0, 'faqmaster.iStatus' => 1]);
                if ($userID) {
                    $Faq->when($userID->iCompanyId, fn($query, $OemCompannyId) => $query->where('faqmaster.iCompanyId', $OemCompannyId))
                        ->when($userID->iOEMCompany, fn($query, $OemCompannyId) => $query->where('faqmaster.iCompanyId', $OemCompannyId));
                }

                if (isset($userID->iCompanyId) && $userID->iCompanyId == 0) {
                    $Faq->whereIn('faqmaster.iCompanyId', function ($query) {
                        $query->select('multiplecompanyrole.iOEMCompany')->from('multiplecompanyrole')->where(["userid" => Auth::user()->id]);
                    });
                } else if (isset($userID->iOEMCompany) && $userID->iOEMCompany == 0) {
                    $Faq->whereIn('faqmaster.iCompanyId', function ($query) {
