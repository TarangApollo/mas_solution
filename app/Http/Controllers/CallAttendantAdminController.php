...
class CallAttendantAdminController extends Controller
{
    ...
    public function index(Request $request)
    {
        if (Auth::User()->role_id == 3) {
            ...
            $Company_Master = CompanyMaster::where(['companymaster.isDelete' => 0, 'companymaster.iStatus' => 1])
                    ->orderBy('strOEMCompanyName', 'ASC')
                    ->where('iCompanyId', Session::get('CompanyId'))
                    ->first();
            ...
            return view('call_attendant.index', compact('CompanyMaster', 'statemasters', 'citymasters','Company_Master'));
        } else {
            return redirect()->route('home');
        }
    }
    ...
    public function checkCustNumber(Request $request)
    {
        ...
        $CompanyMaster = CompanyMaster::where(['companymaster.isDelete' => 0, "iCompanyId" => Session::get('CompanyId')])
                ->orderBy('strOEMCompanyName', 'ASC')
                ->first();
        if(isset($CompanyMaster->iAllowedCallCount) && $CompanyMaster->iAllowedCallCount == 0){
            $options = [
                'NO',
                'ID',
                'STATUS',
            ...
            foreach ($selectedOptions as $option) {
                switch ($option) {
                    ...
                    case 'ID':
                        ...
                        $ticketHtml .= '<td>
                            <a href="../complaint/info/' . $ticket->iTicketId . '" title="Info" class="table-action fs-6 text-secondary text-decoration-underline">
                        ' . $strTicketUniqueID ?? "" . '</a></td>';
                        break;
                    case 'STATUS':
                        $ticketHtml .= '<td>' . $ticket->ticketName . '</td>';
                        break;
                    case 'CALL COUNT':
                        $msg = "";
                        if(isset($ticket->iPhoneStatus) && $ticket->iPhoneStatus == 1){
                            $msg .= "Phone : ".$ticket->iPhoneCount;
                        }
                        if(isset($ticket->iWhatsAppStatus) && $ticket->iWhatsAppStatus == 1){
                            $msg .= "<br /> WhatsApp :".$ticket->iWhatsAppCount;
                        }
                        $ticketHtml .= '<td>'.$msg.'</td>';
                        break;
                    case 'CONTACT NAME':
    ...
    public function autofillCustNumberListComplain(Request $request)
    {
        ...
        $search_company = Session::get('CompanyId');
    ...
    public function create(Request $request)
    {
        if (Auth::User()->role_id == 3) {
            ...
            $iStatus = 1;
            $dataInfo = array();
            $iCompanyId = $request->input('OemCompannyId', session('CompanyId', 0));
            if ($request->iCompanyProfileId == 'Other' && $request->othrcompanyprofile != '') {
                ...
                $CompanyMaster = CompanyMaster::where(['companymaster.isDelete' => 0, 'companymaster.iStatus' => 1, 'iCompanyId' => $iCompanyId])->first();
                $company_Info = CompanyInfo::where('iCompanyId', '=', $iCompanyId)->first();
            ...
            //     ->max('iOEMTicketId');
            $CompanyMaster = CompanyMaster::where(['companymaster.isDelete' => 0, 'companymaster.iStatus' => 1, 'iCompanyId' => $iCompanyId])
                ->first();
            $maxId = DB::table('ticketmaster')
            ...
            if(isset($CompanyMaster->iAllowedCallCount) && $CompanyMaster->iAllowedCallCount == 1){
                $iCallThrough = $request->iCall_Through ?? [];
                $dataArr = array(
                    //"iOemCompannyId" => $request->OemCompannyId,
                    "iOemCompannyId" => $iCompanyId,
                    "iTicketId" => $iTicketId,
                    "iUserId" => $session,
                    "iPhoneStatus"     => in_array("Phone", $iCallThrough) ? 1 : 0,
                    "iWhatsAppStatus"  => in_array("Whatsapp", $iCallThrough) ? 1 : 0,
                    // "iPhoneCount" => $request->iPhoneCount ? $request->iPhoneCount : 0,
                    // "iWhatsAppCount" => $request->iWhatsAppCount ? $request->iWhatsAppCount : 0,
                    "iPhoneCount"      => in_array("Phone", $iCallThrough) ? ($request->iPhoneCount ?? 0) : 0,
                    "iWhatsAppCount"   => in_array("Whatsapp", $iCallThrough) ? ($request->iWhatsAppCount ?? 0) : 0,

                    "iStatus" => 1,
                    "isDelete" => 0,
                    "strIP" => $request->ip(),
                );
                CompanyCallCount::create($dataArr);
            }
            ...
            if ($levelId == 2 && $level2Exe != 0) {
                ...
                $companyInfo = CompanyInfo::where('iCompanyId', '=', $iCompanyId)->first();
            ...
            $messagemaster = DB::table('messagemaster')
                ...
                ->where(['iCompanyId' => $iCompanyId, "iStatusId" => $iStatusId])
                ->first();

            $ticketstatus = DB::table('ticketstatus')->where('istatusId', $request->iTicketStatus)->first();
            $companyclient = DB::table('companyclient')->where('iCompanyClientId',$companyId)->first();
            if (!empty($companyInfo)) {
                $data = array(
                    "CompanyName" => $companyInfo->strCompanyName,
                    "CustomerCompanyName" => $companyclient->CompanyName ?? '',
            ...
            if (!empty($messagemaster)) {
                ...
                if ($messagemaster->toCompany != 0) {
                    ...
                    $CompanyMaster = CompanyMaster::where(['isDelete' => 0, 'iStatus' => 1, 'iCompanyId' => $iCompanyId])->first();
                    // $CompanyMaster = CompanyMaster::where(['companymaster.isDelete' => 0, 'companymaster.iStatus' => 1, 'iCompanyId' => $request->OemCompannyId])
                    //     ->first();
        ...
        if (Auth::User()->role_id == 3) {
            ...
            if ($request->iCompanyProfileId == 'Other' && $request->othrcompanyprofile != '') {
                $SendEmailDetails = DB::table('sendemaildetails')
                    ->where(['id' => 10])
                    ->first();
                // $CompanyMaster = CompanyMaster::where(['companymaster.isDelete' => 0, 'companymaster.iStatus' => 1, 'iCompanyId' => $request->OemCompannyId])->first();
                // $company_Info = CompanyInfo::where('iCompanyId', '=', $request->OemCompannyId)->first();
                $CompanyMaster = CompanyMaster::where(['companymaster.isDelete' => 0, 'companymaster.iStatus' => 1, 'iCompanyId' => $iCompanyId])->first();
                $company_Info = CompanyInfo::where('iCompanyId', '=', $iCompanyId)->first();
            ...
            $TicketMst = DB::table('ticketmaster')->where('iTicketId', $request->iTicketId)->first();
            
            $company_master = CompanyMaster::where(['companymaster.isDelete' => 0, 'companymaster.iStatus' => 1, 'iCompanyId' => $TicketMst->OemCompannyId])
                    ->first();
            if($company_master->iAllowedCallCount == 1){
                $iCallThrough = $request->iCall_Through ?? [];
                $dataArr = array(
                    "iUserId" => $session,
                    // "iPhoneCount" => $request->iPhoneCount ? $request->iPhoneCount : 0,
                ...
                $CompanyCallCount = CompanyCallCount::where(['iTicketId' => $request->iTicketId,"iOemCompannyId" => $TicketMst->OemCompannyId])->first();
                if(empty($CompanyCallCount)){
                    $dataArr['iTicketId'] = $request->iTicketId;
                    $dataArr['iOemCompannyId'] = $TicketMst->OemCompannyId;
                    CompanyCallCount::create($dataArr);
                }else {
                    CompanyCallCount::where(['iTicketId' => $request->iTicketId,"iOemCompannyId" => $TicketMst->OemCompannyId])->update($dataArr);
                }
            }
            ...
            if ($TicketMst->LevelId) {
                if (Session::get('exeLevel') == 2)
                    $levelId = 2;
                else
                    $levelId = $TicketMst->LevelId;
            } else {
                if (Session::get('exeLevel') == 2)
                    $levelId = 2;
            }
            ...
            $companyInfo = CompanyInfo::where('iCompanyId', '=', $TicketMst->OemCompannyId)->first();
            
            $iStatusId = 0;
            if ($TicketMst->iTicketStatus == 0 || $TicketMst->iTicketStatus == 1 || $TicketMst->iTicketStatus == 3) {
                $iStatusId = 0;
            } else {
                $iStatusId = 1;
            }
            $messagemaster = DB::table('messagemaster')
                ->where(['iCompanyId' => $TicketMst->OemCompannyId, "iStatusId" => $iStatusId])
                ->first();
        
            $ticketstatus = DB::table('ticketstatus')->where('istatusId', $TicketMst->iTicketStatus)->first();
            $companyclient = DB::table('companyclient')->where('iCompanyClientId',$companyId)->first();
            ...
            if (!empty($companyInfo)) {
                $data = array(
                    ...
                    "CustomerCompanyName" => $companyclient->CompanyName ?? '',
                    "Message" => $messagemaster->strMessage ?? "",
                    "TicketID" => $TicketMst->strTicketUniqueID ?? str_pad($TicketMst->iTicketId, 4, "0", STR_PAD_LEFT),
                    "TicketStatus" => $ticketstatus->ticketName,
                    "CustomerMobile" => $TicketMst->CustomerMobile,
                    "CustomerName" => $TicketMst->CustomerName,
                    "TicketOpenDate" => date('d/m/Y, H:i:s'),
                    "CompanyEmail" => $companyInfo->EmailId,
                    "CompanyMobile" => $companyInfo->ContactNo,
                    "instaLink" => $companyInfo->instaLink,
            ...
            $ToEmail = array();
            // echo 'iCompanyId :'.  $TicketMst->OemCompannyId . ",iStatusId :" . $iStatusId;
            // dd($messagemaster);
            if (!empty($messagemaster)) {
                if ($messagemaster->toCustomer != 0) {
                    if ($TicketMst->CustomerEmail != "") {
                        $ToEmail[] = $TicketMst->CustomerEmail;
                    }
                }
                // dd($TicketMst->CustomerEmail);
                if ($messagemaster->toCompany != 0) {
                    $CompanyMaster = CompanyMaster::where(['isDelete' => 0, 'iStatus' => 1, 'iCompanyId' => $TicketMst->OemCompannyId])->first();
                    // $CompanyMaster = CompanyMaster::where(['companymaster.isDelete' => 0, 'companymaster.iStatus' => 1, 'iCompanyId' => $request->OemCompannyId])
                    //     ->first();
                    // if ($request->CustomerEmailCompany != "") {
                    //     $ToEmail[] = $request->CustomerEmailCompany;
                ...
                if ($messagemaster->toExecutive != 0) {
                    if ($levelId == 2) {
                        if ($TicketMst->iLevel2CallAttendentId != "") {

                            $CallAttendent = CallAttendent::where(['isDelete' => 0, 'iStatus' => 1, 'iCallAttendentId' => $TicketMst->iLevel2CallAttendentId])->first();
                            if($CallAttendent){
                                $ToEmail[] = $CallAttendent->strEmailId;
                            }
                        } else {
